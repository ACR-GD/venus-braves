...
def process_fhm(fhm_path, ite_dir, out_dir):
    ...
    if entry0[0] == 0x80:
        ...
        start = 0x50 # Guess based on option.fhm
        
    comp_clut = entry0[start:]
    try:
        raw_clut = lzss_decompress(comp_clut)
        print(f"Decompressed CLUT size: {len(raw_clut)}")
...
process_fhm('cdimage_unpacked/seven_data_link/futa/screen/option.fhm', 'ite_out/option', 'ite_out/option_decoded_lzss')
process_fhm('cdimage_unpacked/seven_data_link/futa/screen/naming.fhm', 'ite_out/naming', 'ite_out/naming_decoded_lzss')
